...
![regression partition](predictive_workinggroup/images/rp1.png)
...
rp1<-caret::train(y=y, x=x,
           metric="ROC",
           method ="rf",
          tuneLength=5, #try 20 random values of the tuning parameters
           trControl=fitctrl,
...
rp_vis <- rpart(y ~ ., data = data.frame(x, y), method = "class")
rp_pruned <- prune(rp_vis, cp = rp_vis$cptable[which.min(rp_vis$cptable[,"xerror"]), "CP"])
prp(rp_pruned, type=4, extra=4, main="Classification tree for using modern contraception")
...
SHAP (SHapley Additive exPlanations) values provide a unified measure of feature importance that shows how much each predictor contributes to an individual prediction, and in which direction.
...
pfun <- function(object, newdata) {
  predict(object, newdata = newdata, type = "prob")[, "mod"]
}

# Compute SHAP values on training data (subset for speed)
set.seed(1115)
shap_vals <- explain(
  object    = rp1,
  X         = x,
...
shap_imp <- shap_imp[order(shap_imp$mean_shap, decreasing = TRUE), ]

ggplot(head(shap_imp, 15), aes(x = reorder(feature, mean_shap), y = mean_shap)) +
  geom_col(fill = "steelblue") +
  coord_flip() +
  labs(title = "Mean |SHAP| — top 15 features",
...
classify_metrics(table(predt1, yt))
```
