...
I would suggest you read section 5.1 of [Introduction to Statistical Learning](https://www.statlearning.com/) to get a full treatment of this topic.
...
model.dat2<-model.dat%>%
  ...
  dplyr::select(caseid, region, modcontra,age, age2,livchildren, educ, knowmodern, rural, wantmore, wt)
...
K-Nearest Neighbors (KNN) is a non-parametric classification method. For each new observation, the algorithm finds the k most similar (nearest) observations in the training data and assigns the class by majority vote among those k neighbors.

Key considerations:
- The choice of k is a tuning parameter — small k is more flexible (can overfit), large k is smoother
- Features should be centered and scaled since distance is sensitive to variable scale
- KNN can be slow on large datasets since it requires computing distances to all training points
...
dtrain<-datmat[train,c(-1,-5, -9, -16)]
dtest<-datmat[-train,c(-1,-5, -9, -16)]
...
### Set up caret for 5-fold cross-validation
...
Here we compare a logistic regression, random forest, and KNN model using cross-validation.
...
rp3<-caret::train(modcontra~.,
           data=dtrain,
           metric="ROC",
           method ="knn",
           tuneGrid=expand.grid(k = c(30, 35, 40, 45, 50, 55, 60)),
           trControl=fitctrl)
...
confusionMatrix(data = predict(rp1, newdata=dtrain),dtrain$modcontra, positive = "Modcontra" )
confusionMatrix(data = predict(rp2, newdata=dtrain),dtrain$modcontra, positive = "Modcontra" )
confusionMatrix(data = predict(rp3, newdata=dtrain),dtrain$modcontra, positive = "Modcontra" )
...
### ROC curve
The ROC curve can be shown for the KNN model:

```{r}

library(pROC)
# Select a parameter setting for KNN
selectedIndices <- rp3$pred$k==rp3$bestTune$k
# Plot:
plot.roc(rp3$pred$obs[selectedIndices], rp3$pred$Modcontra[selectedIndices], grid=T,
         main="ROC Curves: KNN (black) vs Logistic (blue)")
plot.roc(rp1$pred$obs, rp1$pred$Modcontra, add=T, col="blue")

#Value of AUC
auc(rp3$pred$obs[selectedIndices],  rp3$pred$Modcontra[selectedIndices])
auc(rp1$pred$obs, rp1$pred$Modcontra)
...
predrp3<-predict(rp3, newdata=dtest)
confusionMatrix(data = predrp3,dtest$modcontra, positive = "Modcontra" )
...
3. Compare multiple model types on the same data using consistent resampling
