...
library(pROC)
# Check what columns are available in rp1$pred
names(rp1$pred)
rp1$bestTune

# For glmnet, use lambda instead of cp
...
# Check data
table(rp1_obs_numeric)
table(gl1_obs_numeric)

# Plot:
plot.roc(gl1_obs_numeric, gl1$pred$Modcontra, grid=T)
plot.roc(rp1_obs_numeric, rp1$pred$Modcontra[selectedIndices], add=T, col="blue")

#Value of ROC and AUC
roc(rp1_obs_numeric,  rp1$pred$Modcontra[selectedIndices])
roc(gl1_obs_numeric, gl1$pred$Modcontra)
auc(rp1_obs_numeric,  rp1$pred$Modcontra[selectedIndices])
auc(gl1_obs_numeric, gl1$pred$Modcontra)

roc.test(roc(gl1_obs_numeric, gl1$pred$Modcontra),roc(rp1_obs_numeric,  rp1$pred$Modcontra[selectedIndices]) )
```
